...
import { getBillingClientById } from "@/lib/billing";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const billing = await getBillingClientById(providerId);

    if (!billing) {
      return NextResponse.json({ error: "ספק לא נמצא" }, { status: 404 });
    }

    const result = await billing.client.testConnection();
    ...
    if (result.success) {
      ...
      console.error("Billing connection test failed:", errorMsg);
      await prisma.billingProvider.update({
        where: { id: billing.provider.id },
