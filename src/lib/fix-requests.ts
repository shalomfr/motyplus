...
## מפת האתר — אתה מכיר את הקוד!

### עמודים ראשיים (CRM):
- **דף הבית** / — דשבורד עם סטטיסטיקות, גרף הכנסות, פעילות אחרונה
- **לקוחות** /customers — טבלת לקוחות, חיפוש, סינון, ייצוא. כרטיס לקוח /customers/[id]
- **עדכונים** /updates — ניהול גרסאות עדכון (V3-V5+), העלאת קבצים, שליחה ללקוחות
- **מיילים** /emails — תבניות מייל, עורך בלוקים, שליחה בודדת ובאלק
- **לידים** /leads — pipeline מכירות, כרטיסי לידים, המרה ללקוח
- **נתונים** /data — ניהול אורגנים וסוגי סטים
- **מבצעים** /promotions — קופונים והנחות
- **חשבונות** /accounting — חשבוניות, תשלומים, דוחות
- **משימות** /tasks — לוח קנבן (רעיון→תכנון→בביצוע→הושלם)
- **רשימת עבודה** /work-orders — הזמנות עבודה
- **יומן פעילות** /activity-log — לוג כל הפעולות במערכת
- **הגדרות** /settings — הגדרות מערכת, חיבורי סליקה
- **התחברות** /login — עמוד כניסה

### רכיבי UI חשובים:
- **Sidebar** (src/components/layout/Sidebar.tsx) — תפריט צד ימין עם כל הניווט
- **Header** (src/components/layout/header.tsx) — כותרת עליונה
- **AnimatedLogo** — לוגו מונפש
- **כרטיס לקוח** — customer-card, customer-form, customer-table
- **עורך מייל** — email-editor, block-editor, rich-email-editor
- **Pipeline לידים** — lead-pipeline, lead-card
- **דשבורד** — stats-cards, revenue-chart, recent-activity

### סגנון:
- Tailwind CSS + Shadcn/UI + Lucide icons
- צבע ראשי: כחול (#2563eb), גבולות: #e8ecf4
- פונט: Heebo (עברית), כיוון RTL
- כרטיסים לבנים עם rounded-2xl ו-border
